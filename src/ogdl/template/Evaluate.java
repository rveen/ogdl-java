 ...
 * Supported types:
 * 
 * Boolean Long Double String Graph
 * 
 * $Id$ Initial date: 2002
 * 
 * TODO (XXX):
 * 
 * - Late evaluation of numbers (only in math context!), else when printing text
 * with only numbers, leading zeros are eliminated.
 * 
 * - !obj needed ?
 * 
 */
...
public class Evaluate {
	...
	public static boolean evalBoolean(IGraph expr, IGraph context)
			throws Exception {
	...
	public static Object eval(IGraph expr, IGraph context) throws Exception {
		/**
		 * this flag is set to true when the name of the current node is not a
		 * value but a directive (such as Types.EXPR, etc).
		 */
		...
		if (s.equals(Types.VAR) || s.equals(Types.VARE)) {
			 ...
			 * 
			 * The result of the evaluation is used as expression to a new
			 * evaluation.
			 */
	...
	static IGraph evalGraph(IGraph e, IGraph g) throws Exception {
		...
		if (s.equals(Types.EXPR) || s.equals(Types.VAR) || s.equals(Types.VARE)
				|| s.equals(Types.GROUP)) {
		...
		for (int i = 0; i < e.size(); i++) {
			...
			while (transparent(n) && n.size() == 1)
				// XXX: Integrate into evalGraph
				n = n.get(0);
	...
	/**
	 * If the object is IGraph without subnodes, it looks at the name and
	 * returns:
	 * 
	 * getValue() if Types.OBJECT Long if integer number Double if floating
	 * point Boolean if 'true' or 'false'
	 * 
		...
		if (a instanceof Double) {
			...
			if (b instanceof Long)
				return op ? new Double(((Double) a).doubleValue()
						- ((Long) b).longValue()) : new Double(
						((Double) a).doubleValue() + ((Long) b).longValue());
			else if (b instanceof Double)
				return op ? new Double(((Double) a).doubleValue()
						- ((Double) b).doubleValue())
						: new Double(((Double) a).doubleValue()
								+ ((Double) b).doubleValue());
			...
			if (b instanceof Double)
				return op ? (new Double(-((Double) b).doubleValue() + (Long) a)
						.longValue()) : new Double(((Double) b).doubleValue()
						+ ((Long) a).longValue());
			else if (b instanceof Long)
				return op ? new Long(((Long) a).longValue()
						- ((Long) b).longValue()) : new Long(
						((Long) b).longValue() + ((Long) a).longValue());
		} else if (a instanceof String) {
			if (a == null && b == null)
				return "";
			if (a == null)
	...
	static Object _mul(Object a, Object b, boolean op) throws Exception {
		...
		if (a instanceof Double) {
			...
			if (b instanceof Double)
				return op ? new Double(((Long) a).longValue()
						/ ((Double) b).doubleValue()) : new Double(
						((Long) a).longValue() / ((Double) b).doubleValue());
			else if (b instanceof Long)
				return op ? new Long(((Long) a).longValue()
						/ ((Long) b).longValue()) : new Long(
						((Long) a).longValue() * ((Long) b).longValue());
		}
		return null;
	}
	...
	/**
	 * Test if b appears in a. For that both should be strings.
	 */

	private static Object _contains(Object a, Object b) throws Exception {
		if (a == null || b == null)
			return new Boolean(false);
		...
		while (path != null) {
			...
			if (Types.INDEX.equals(e)) {
				...
				if (o instanceof String) {
					...
					throw new Exception("not a valid index " + e);
				}
			...
			if (tmpNode == null) {
				...
				if ("_name".equals(e)) {
					...
					/*
					 * during evaluation, values are possibly converted to
					 * objects.
					 */
			...
			prev = path;
			if (path.size()==0 || path.getName().equals("!i")) // XXX 
				break;
			path = path.get(0);
		}
	...
	public static void _set(IGraph path, Object value, IGraph g)
			throws Exception {
		IGraph node = g, tmp;
		...
		while (Types.EXPR.equals(path.getName())
				|| Types.PATH.equals(path.getName()))
			path = path.get(0);
			...
			for (int i = 0; i < itf.length; i++) {
				if (itf[i] == IGraph.class || itf[i] == IFunction.class) {
					try {
						...
						try {
							Constructor co = c
									.getConstructor(new Class[] { IGraph.class });
	...
	public static IFunction getFunction(String name, IGraph context) {
		...
		try {
			...
			LocalFunction intro = new LocalFunction(name, context);
			return intro;
		} catch (Exception ex) {
			ex.printStackTrace();
			return null;
	...
	private static IGraph clean(IGraph g) {
		...
		if (s.equals(Types.EXPR) || s.equals(Types.VAR) || s.equals(Types.VARE)
				|| s.equals(Types.GROUP) || s.equals(Types.PATH))
